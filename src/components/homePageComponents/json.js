const GuideLineDetails = [
    {
        title : "How to signup | register",
        guideLine : [
            "Click on signup button",
            "Fill the registration form",
            "Submit form"
        ]
    },

    {
        title : "How to login",
        guideLine : [
            "Click on login button",
            "Enter your email and password",
            "Click on login button"
        ]
    },

    {
        title : "How to verify your email",
        guideLine : [
            "Click on profile",
            "Click verify your account button",
            "An OTP will be sent to your registered email",
            "Enter 4-digit OTP",
            "click verify button"
        ]
    },

    {
        title : "How to fill MCQS",
        guideLine : [
            "You have show only one mcqs",
            "when you fill one mcqs than show another mcqs",
            "You have 3 or 4 options select one of them",
            "click on submit",
            "once you fill mcqs then you have no option to fill again",
        ]
    },
];

const UserFeedBacks = [
    {
        image : './feedbackImage.jpg',
        userName : "Muhammad Bilal",
        feedBack : "Lorem ipsum dolor  id quidem provident! Vel unde velit molestiae adipisci dolorum. Hic."
    },

     {
        image : './feedbackImage.jpg',
        userName : "Muhammad Bilal",
        feedBack : "Lorem ipsum dolor sit amet consectetur adipisicing elit. Consequuntur, voluptatem non delectus accusantium facilis velit aperiam tempore. Porro aperiam voluptatum id quidem provident! Vel unde velit molestiae adipisci dolorum. Hic."
    },

    {
        image : './feedbackImage.jpg',
        userName : "Muhammad Bilal",
        feedBack : "Lorem ipsum dolor sit amet consectetur adipisicing elit. Consequuntur, voluptatem non delectus accusantium facilis velit aperiam tempore. Porro aperiam voluptatum id quidem provident! Vel unde velit molestiae adipisci dolorum. Hic."
    },

    {
        image : './feedbackImage.jpg',
        userName : "Muhammad Bilal",
        feedBack : "Lorem ipsum Vel unde velit molestiae adipisci dolorum. Hic."
    },
];

const questions= [
  {
    question: "What is the capital of France?",
    answers: ["Madrid", "Berlin", "Paris", "Rome"],
    correctAnswer: 2
  },
  {
    question: "Which planet is known as the Red Planet?",
    answers: ["Earth", "Mars", "Jupiter", "Venus"],
    correctAnswer: 1
  },
  {
    question: "Who wrote the national anthem of Pakistan?",
    answers: ["Allama Iqbal", "Liaquat Ali Khan", "Hafeez Jullundhri", "Faiz Ahmad Faiz"],
    correctAnswer: 2
  },
  {
    question: "What is the largest ocean on Earth?",
    answers: ["Indian Ocean", "Arctic Ocean", "Atlantic Ocean", "Pacific Ocean"],
    correctAnswer: 3
  },
  {
    question: "Which gas do plants absorb from the atmosphere?",
    answers: ["Oxygen", "Carbon Dioxide", "Nitrogen", "Hydrogen"],
    correctAnswer: 1
  },
  {
    question : "Lorem ipsum dolor sit, amet consectetur adipisicing elit.Lorem ipsum dolor sit, amet consectetur adipisicing elit. Consequuntur odit dolorum, accusantium consectetur magnam magni inventore quisquam eaque minus ipsa asperiores alias incidunt fuga. Accusamus rerum aliquam aut inventore ex.",
    answers: ["Oxygen", "Carbon Dioxide", "Nitrogen", "Hydrogen"],
  }
];

export {GuideLineDetails, UserFeedBacks, questions};